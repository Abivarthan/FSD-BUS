...
const seedData = async () => {
    try {
        ...
        console.log('👤 Seeding Admin and Customer...');
        ...
        const fuelLogs = [];
        // Each bus starts with a different base odometer (realistic fleet diversity)
        const baseOdometers = [42000, 38500, 55000, 61000, 29000];
        for (let b = 0; b < buses.length; b++) {
            const bus = buses[b];
            let odo = baseOdometers[b];
            // i=4 is oldest date, i=0 is newest — build from oldest to newest
            const entries = [];
            for (let i = 4; i >= 0; i--) {
                const date = new Date(now);
                date.setDate(now.getDate() - (i * 6));
                const kmDriven = 280 + Math.floor(Math.random() * 120); // 280–400 km per 6 days
                odo += kmDriven;
                entries.push({
                    vehicle_id: bus._id,
                    driver_id: drivers[Math.floor(Math.random() * drivers.length)]._id,
                    date: date,
                    fuel_quantity_liters: parseFloat((40 + Math.random() * 20).toFixed(1)),
                    fuel_cost: parseFloat((2500 + Math.random() * 1000).toFixed(2)),
                    odometer_reading: odo,
                    fuel_station: 'Bharat Petroleum, Anna Salai'
                });
            }
            fuelLogs.push(...entries);
        }
