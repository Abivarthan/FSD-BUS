...
exports.createBooking = async (req, res, next) => {
  try {
    const { routeId, bookingDate, seatNumber, departureTime, mealPreference } = req.body;
    
    const route = await Route.findById(routeId);
    if (!route) return res.status(404).json({ success: false, message: 'Route not found' });

    // Auto-assign a vehicle matching the route's bus_type
    ...
    const totalPrice = route.price * seats.length;

    const booking = new Booking({
      user: req.user.id,
      route: routeId,
      vehicle: matchingVehicle?._id || null,
    ...
    await booking.save();
    logger.info(`New booking created by user ${req.user.id} for route ${routeId} (Seats: ${seatString}, Vehicle: ${matchingVehicle?.registration_number || 'unassigned'})`);
