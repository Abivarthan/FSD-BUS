  ...
  async getLivePositions() {
    try {
      ...
      const realPositions = latestPositions
        ...
        .map(p => ({ ...p, _id: p.vehicle_id }));

      const returnedPositions = [...simulatedPositions.map(s => ({ ...s, _id: s.vehicle_id })), ...realPositions];
      const returnedVehicleIds = new Set(returnedPositions.map(p => p.vehicle_id.toString()));

      const allVehicles = await Vehicle.find();
      const fallbackPositions = allVehicles
        .filter(v => !returnedVehicleIds.has(v._id.toString()))
        .map((v, i) => ({
            _id: v._id,
            vehicle_id: v._id,
            registration_number: v.registration_number,
            latitude: 13.0827 - (i * 0.01), // stagger south
            longitude: 80.2707 - (i * 0.01), // stagger west (inland)
            speed: 0,
            status: 'idle',
            driver: 'Unassigned'
        }));

      return [...returnedPositions, ...fallbackPositions];
    } catch (error) {
