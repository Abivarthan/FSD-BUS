  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <h1 className="page-title text-white">Customer Bookings</h1>
            ...
            {loading ? <tr><td colSpan="6" className="text-center py-10">Loading...</td></tr> : 
             ...
             bookings.map(b => (
              <tr key={b._id}>
                ...
                <td>
                  <div className="font-semibold text-white">{b.route?.name}</div>
                  <div className="text-[10px] text-gray-400">{b.departure_time}</div>
                </td>
                <td>{new Date(b.booking_date).toLocaleDateString()}</td>
                <td><span className="px-2 py-0.5 bg-gray-100 rounded text-xs font-bold text-slate-500">{b.seat_number}</span></td>
                <td className="font-bold text-primary">${b.price}</td>
                <td>
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                    b.status === 'Confirmed' ? 'bg-green-100 text-green-600' : 
                    b.status === 'Cancelled' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-primary'
                  }`}>
                    {b.status}
                  </span>
                </td>
