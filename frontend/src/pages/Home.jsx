...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-surface">
      {/* Navigation */}
      <nav className="flex items-center justify-between px-8 py-6 bg-surface border-b border-slate-800">
        ...
        <div className="flex items-center gap-6">
          {user ? (
            ...
            <>
              <Link to="/login" className="text-slate-400 font-medium hover:text-primary transition-colors">Login</Link>
      ...
      <section className="relative pt-20 pb-32 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-8 relative z-10">
          ...
          {/* Search Card */}
          <div className="max-w-4xl mx-auto bg-surface-card p-8 rounded-[32px] shadow-2xl shadow-slate-950/50 border border-slate-800 mb-20 animate-fade-in-up">
            <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Origin</label>
                <div className="relative group">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-hover:text-primary transition-colors">📍</span>
                  <select 
                    className="w-full pl-12 pr-10 py-4 bg-slate-900 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 transition-all font-medium text-white appearance-none cursor-pointer hover:bg-slate-800"
                    value={search.origin}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest text-slate-500 ml-1">Destination</label>
                <div className="relative group">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-hover:text-primary transition-colors">🏁</span>
                  <select 
                    className="w-full pl-12 pr-10 py-4 bg-slate-900 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 transition-all font-medium text-white appearance-none cursor-pointer hover:bg-slate-800"
            ...
            {error && (
              <div className="mt-4 p-4 bg-red-500/10 text-red-500 rounded-xl text-sm font-medium border border-red-500/20 animate-fade-in">
                {error}
              </div>
            )}
          </div>
          ...
          <div className="max-w-4xl mx-auto mb-16 grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in-up delay-100">
            {[
              { type: 'AC Sleeper', fare: '₹1500 – ₹2000', icon: '❄️', color: 'blue' },
              { type: 'Non-AC Sleeper', fare: '₹1000', icon: '🛌', color: 'orange' },
              { type: 'Seater (Cushion)', fare: '₹300 – ₹500', icon: '💺', color: 'green' },
            ].map(f => (
              <div key={f.type} className="bg-surface-card p-6 rounded-2xl border border-slate-800 shadow-sm flex items-center gap-4">
                <div className="text-3xl">{f.icon}</div>
                <div>
                  <h3 className="font-bold text-white">{f.type}</h3>
                  <p className="text-sm text-slate-400">{f.fare}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {routes.length > 0 && (
            <div className="max-w-4xl mx-auto space-y-4 animate-fade-in">
              <h2 className="text-2xl font-display font-bold text-white mb-6">Available Buses</h2>
              {routes.map(route => (
                <div key={route._id} className="group bg-surface-card p-6 rounded-2xl border border-slate-800 hover:border-primary/30 hover:shadow-glow-primary transition-all flex flex-col md:flex-row items-center justify-between gap-6 cursor-pointer" onClick={() => navigate(`/booking/${route._id}`)}>
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-slate-400 text-sm">Departure: {route.schedule[0]?.time} | <span className="font-semibold text-primary">{route.bus_type}</span></p>
                      <div className="flex gap-4 mt-1">
                        <span className="text-[11px] font-bold text-slate-500 flex items-center gap-1">🛣️ {route.distance || '—'}</span>
                        <span className="text-[11px] font-bold text-slate-500 flex items-center gap-1">⏱️ {route.duration || '—'}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-8">
                    <div className="text-right">
                      <p className="text-xs font-bold text-slate-500 uppercase">Fare</p>
                      <p className="text-2xl font-display font-black text-primary">₹{route.price}</p>
                    </div>
                    <button className="px-6 py-3 bg-slate-800 text-white rounded-xl font-bold hover:bg-primary transition-colors">Book Now</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-center">
            {[
              ...
              <div key={f.title} className="p-8 bg-surface-card rounded-3xl border border-slate-800 shadow-sm hover:shadow-glow-primary transition-all">
