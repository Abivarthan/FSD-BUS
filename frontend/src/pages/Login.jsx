...
export default function Login() {
  const [form, setForm] = useState({ email: 'admin@busms.com', password: 'Admin@123' });
  ...
  return (
    <div className="min-h-screen bg-surface flex">
      {/* Left panel */}
      <div className="hidden lg:flex lg:w-1/2 bg-surface-card border-r border-slate-800 flex-col items-center justify-center p-12 relative overflow-hidden">
        {/* Background grid */}
        ...
        <div className="w-full max-w-md animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <button
                type="button"
                onClick={() => setForm({ email: 'admin@busms.com', password: 'Admin@123' })}
                className="px-4 py-2 bg-slate-800 border border-slate-700 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:bg-primary/5 hover:text-primary hover:border-primary/20 transition-all flex flex-col items-center gap-1"
              >
              ...
              <button
                type="button"
                onClick={() => setForm({ email: 'customer@busms.com', password: 'Customer@123' })}
                className="px-4 py-2 bg-slate-800 border border-slate-700 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:bg-primary/5 hover:text-primary hover:border-primary/20 transition-all flex flex-col items-center gap-1"
              >
                <span>👤 Customer Role</span>
                <span className="text-[8px] opacity-60 normal-case font-medium">customer@busms.com</span>
              </button>
            </div>
          ...
          <div className="mt-10 text-center">
            <p className="text-slate-400 text-sm">Don't have an account? <Link to="/register" className="text-primary font-bold hover:underline">Sign up for free</Link></p>
          </div>
        </div>
      </div>
    </div>
