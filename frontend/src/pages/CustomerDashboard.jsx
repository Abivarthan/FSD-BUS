    ...
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400 text-lg">Manage your trips and track live locations</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={stat.label} className="relative bg-surface-card p-6 rounded-[28px] border border-slate-800 shadow-sm hover:shadow-glow-primary transition-all group overflow-hidden">
      ...
      {/* Filter Tabs */}
      <div className="flex items-center gap-3 bg-slate-900/50 p-2 rounded-2xl w-fit border border-slate-800">
        {filters.map(f => (
          <button
            key={f.value}
          ...
          >
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-full ${
              filter === f.value ? 'bg-primary/20 text-primary' : 'bg-slate-800 text-slate-500'
            }`}>{f.count}</span>
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-surface-card p-16 rounded-[40px] text-center border border-slate-800 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-6">
          {filteredBookings.map(booking => (
            <div key={booking._id} className="bg-surface-card rounded-[32px] border border-slate-800 shadow-sm p-8 hover:shadow-glow-primary transition-all group">
                ...
                <div className="flex items-center gap-6 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-3 flex flex-wrap gap-3">
                      <span className="px-3 py-1 bg-slate-900 rounded-full text-[10px] font-bold text-slate-400 uppercase tracking-widest border border-slate-800">
                      ...
                      <span className="px-3 py-1 bg-slate-900 rounded-full text-[10px] font-bold text-slate-400 uppercase tracking-widest border border-slate-800">
                        🚌 {booking.route?.bus_type || 'Bus'}
                      </span>
                      <span className="px-3 py-1 bg-slate-900 rounded-full text-[10px] font-bold text-slate-400 uppercase tracking-widest border border-slate-800">
                        🛣️ {booking.route?.distance || '—'}
                      </span>
                      <span className="px-3 py-1 bg-slate-900 rounded-full text-[10px] font-bold text-slate-400 uppercase tracking-widest border border-slate-800">
                        ⏱️ {booking.route?.duration || '—'}
                      </span>
                    </div>
                  </div>
                </div>
