  ...
  return (
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat) => (
          <div key={stat.label} className="bg-surface-card p-6 rounded-[28px] border border-slate-800 shadow-sm hover:shadow-glow-primary transition-all group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-surface-card p-8 rounded-[32px] border border-slate-800 shadow-sm">
          <h2 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-8 pb-2 border-b border-slate-800 flex items-center gap-2">
          ...
          {editing ? (
            <form onSubmit={handleSave} className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-5 py-4 bg-slate-900 border border-slate-800 rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium text-white"
                  required
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full px-5 py-4 bg-slate-900 border border-slate-800 rounded-2xl focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all font-medium text-white resize-none"
                  rows={3}
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={item.label} className="flex items-start gap-4 p-4 bg-slate-900/50 rounded-2xl">
          ...
          <div className="bg-surface-card p-8 rounded-[32px] border border-slate-800 shadow-sm">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between p-4 bg-slate-900/50 rounded-2xl">
                ...
                <button className="px-4 py-2 bg-slate-800 text-slate-400 rounded-xl text-xs font-bold hover:bg-slate-700 transition-colors">
                  Change
                </button>
              </div>
              <div className="flex items-center justify-between p-4 bg-slate-900/50 rounded-2xl">
              ...
              <div className="flex items-center justify-between p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-emerald-500 text-xs font-medium">{profile?.email}</p>
                  </div>
                </div>
          ...
          <div className="bg-surface-card p-8 rounded-[32px] border border-slate-800 shadow-sm">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={pref.label} className="flex items-center justify-between p-4 bg-slate-900/50 rounded-2xl">
                  <div>
                    <p className="font-semibold text-white text-sm">{pref.label}</p>
                    <p className="text-slate-500 text-xs">{pref.desc}</p>
                  </div>
                  <div className={`w-12 h-7 rounded-full flex items-center cursor-pointer transition-colors ${pref.enabled ? 'bg-primary justify-end' : 'bg-slate-700 justify-start'}`}>
