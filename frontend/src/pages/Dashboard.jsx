  ...
  if (loading) return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="card p-5 h-28 animate-pulse bg-surface-card" />
        ))}
      </div>
    </div>
  );
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-slate-400 mt-2">Use the sidebar to access your attendance and fuel logs.</p>
        </div>
      </div>
    );
  }
        ...
        <ChartCard title="Monthly Expense Trend" subtitle="Combined categories">
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={charts.monthlyTrend || []}>
              ...
              <Line type="monotone" dataKey="expenses" stroke="#7C3AED" strokeWidth={2} dot={false} name="Total" />
        ...
        <ChartCard title="Fuel Cost by Vehicle" subtitle="This month">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={charts.fuelByVehicle || []} layout="vertical">
              ...
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="value" fill="#7C3AED" radius={[0, 4, 4, 0]} name="Fuel Cost" />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
              ...
              return (
                <div key={i}>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="font-semibold text-slate-300">{v.vehicle}</span>
                    <span className="text-slate-400">{fmtCurrency(v.total_cost)}</span>
                  </div>
            ...
            <PieChart>
              ...
              >
                {(charts.expenseByCategory || []).map((_, i) => (
                  <Cell key={i} fill={COLORS[i % COLORS.length]} stroke="#1E293B" />
                ))}
              </Pie>
              <Tooltip content={<CustomTooltip />} />
