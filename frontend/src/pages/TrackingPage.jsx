...
const TrackingPage = () => {
  ...
  useEffect(() => {
    if (selectedVehicle) {
      fetchRoutePath(selectedVehicle._id);
    } else {
      setRoutePath([]);
    }
  }, [selectedVehicle]);

  const fetchRoutePath = async (vehicleId) => {
    try {
      // Find ongoing trip for this vehicle
      const res = await api.get(`/tracking/live`);
      const vehicleData = res.data.data.find(v => v._id === vehicleId);
      
      // For demo, if we don't have a trip ID in the vehicle data, we use simulation logic
      // In a real app, the vehicle would have an active_trip_id
      if (vehicleData && vehicleData.active_trip_id) {
         const tripRes = await api.get(`/tracking/playback/${vehicleData.active_trip_id}`);
         if (tripRes.data.success) {
           setRoutePath(tripRes.data.data.trip.route_path);
         }
      } else {
         // Fallback: mock route for simulation view
         setRoutePath([[13.0827, 80.2707], [12.9716, 79.1333], [11.9416, 79.4861], [11.0168, 76.9558]]);
      }
    } catch (err) {
  ...
  const fetchLiveLocations = async () => {
    try {
      ...
      if (response.data.success && response.data.data.length > 0) {
        ...
        // Sync selected vehicle with new positions
        if (selectedVehicle) {
          const updated = updatedVehicles.find(v => v._id === selectedVehicle._id);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 px-8 pb-8 flex gap-8">
        ...
        <div className="flex-1 relative">
           <VehicleMap 
              vehicles={selectedVehicle ? [selectedVehicle] : vehicles} 
