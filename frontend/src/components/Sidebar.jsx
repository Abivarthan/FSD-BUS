...
const adminLinks = [
  ...
  { to: '/tracking', label: 'Vehicle Tracking', icon: 'tracking' },
...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed top-0 left-0 h-full w-60 bg-surface-card border-r border-slate-800 z-40 flex flex-col
      ...
      `}>
        ...
        <div className="h-16 flex items-center px-5 border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Fleet Intelligence</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto">
          ...
          {adminLinks.map(({ to, label, icon }) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <span className={`transition-colors ${isActive ? 'text-primary' : 'text-slate-400 group-hover:text-white'}`}>
                    {icons[icon]}
                  </span>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50">
          <div className="flex items-center gap-3 bg-slate-800 p-3 rounded-2xl border border-slate-700 shadow-sm">
